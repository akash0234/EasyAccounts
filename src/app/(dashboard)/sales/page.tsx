...
interface Invoice {
  ...
  paidAmount: number; status: string; customer?: { name: string } | null;
...
export default function SalesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/invoices", {
      ...
      body: JSON.stringify({ type: "SALES", date, dueDate, customerId, items, notes }),
    });
    setLoading(false);
    if (res.ok) {
      setShowForm(false);
                  ...
                  {items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                      <div className="col-span-4">
                        ...
                        <Input value={item.description} onChange={(e) => updateItem(idx, "description", e.target.value)} placeholder="Item name" required />
                      </div>
                      <div className="col-span-2">
                        {idx === 0 && <Label className="text-xs">Qty</Label>}
                        <Input type="number" value={item.quantity} onChange={(e) => updateItem(idx, "quantity", Number(e.target.value))} min={0.01} step={0.01} />
                      </div>
                      <div className="col-span-2">
                        {idx === 0 && <Label className="text-xs">Rate</Label>}
                        <Input type="number" value={item.rate} onChange={(e) => updateItem(idx, "rate", Number(e.target.value))} min={0} />
                      </div>
                      <div className="col-span-2">
                        {idx === 0 && <Label className="text-xs">GST %</Label>}
                        <Input type="number" value={item.gstPercent} onChange={(e) => updateItem(idx, "gstPercent", Number(e.target.value))} min={0} max={28} />
                      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {invoices.length === 0 && (
                  <tr><td colSpan={6} className="p-6 text-center text-gray-500">No sales invoices yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
